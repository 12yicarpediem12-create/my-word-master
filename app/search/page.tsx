  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900 font-sans pb-20">
      ...
      <nav className="bg-white border-b-2 border-gray-200 px-6 py-4 flex flex-col md:flex-row justify-between items-center sticky top-0 z-50 shadow-sm gap-4">
        ...
        <button onClick={() => router.back()} className="text-sm font-bold text-gray-500 hover:text-blue-600 uppercase tracking-widest flex items-center gap-2 shrink-0">
          <span>←</span> Back
        </button>
      </nav>
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-10 sm:py-16">
        ...
        <div className="mb-10 sm:mb-12">
          <p className="text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Search Results</p>
        ...
        {isLoading ? (
          ...
          <div className="space-y-12">
            ...
            {Object.keys(groupedResults).map((langCode: string) => {
              ...
              return (
                <div key={langCode} className="bg-white rounded-[2rem] sm:rounded-[2.5rem] p-6 sm:p-10 border-2 border-gray-200 shadow-sm">
                  ...
                  <div className="divide-y-2 divide-gray-100">
                    ...
                    {words.map((vocab: any) => {
                      ...
                      return (
                        <Link href={`/word/${vocab.id}`} key={vocab.id} className="py-4 hover:bg-gray-50 transition-colors flex flex-col sm:flex-row sm:items-center justify-between group block rounded-2xl px-2 sm:px-4 -mx-2 sm:-mx-4">
                          <div className="flex items-center gap-4 sm:gap-6 mb-2 sm:mb-0">
                            <span className={`w-3 h-3 rounded-full shrink-0 ${vocab.is_remembered ? "bg-green-400" : "bg-orange-400"}`}></span>
                            <div>
          ...
          <div className="bg-white rounded-[2.5rem] p-12 border-2 border-gray-200 shadow-sm text-center">
            ...
            <h2 className="text-2xl font-black text-gray-900 mb-2">No words found</h2>
            <p className="text-gray-500 font-medium">We couldn't find any words matching "{query}".</p>
          </div>
        )}
