...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    async function loadDashboard() {
      ...
      const loadedLangs = langs || [];

      const { data: reviews } = await supabase.from("vocab").select("id").lte("next_review_date", new Date().toISOString());
      setTotalReviews(reviews?.length || 0);

      const { data: allVocab } = await supabase.from("vocab").select("*");
      ...
      const calculatedHeatmap = Object.keys(counts).map(date => ({
        date: date,
        count: counts[date]
      }));
      ...
      const stats = sortedLangs.map((lang) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900 font-sans overflow-x-hidden">
      ...
      {/* 設定サイドバー */}
      {isSettingsOpen && (
        <div className="fixed inset-0 bg-black/30 backdrop-blur-sm z-[60] transition-opacity" onClick={() => setIsSettingsOpen(false)} />
      )}
      <div className={`fixed z-[70] bg-white shadow-2xl transition-transform duration-300 ease-in-out top-0 right-0 h-full w-72 md:left-0 md:w-full md:h-auto md:border-b-2 md:border-gray-200 ${isSettingsOpen ? "translate-x-0 md:translate-y-0" : "translate-x-full md:translate-x-0 md:-translate-y-full"}`}>
        <div className="p-8 md:p-6 h-full flex flex-col md:flex-row gap-8 md:gap-12 md:items-center">
          <div className="flex justify-between items-center md:w-auto">
            <h2 className="text-2xl font-black text-gray-900 flex items-center gap-2"><span>⚙️</span> Settings</h2>
      ...
      <nav className="bg-white border-b-2 border-gray-200 px-6 py-4 flex flex-col md:flex-row justify-between items-center sticky top-0 z-50 shadow-sm gap-4">
         <div className="text-3xl font-black tracking-tighter text-blue-600 w-full md:w-auto flex justify-between items-center">
           WordMaster.
           <button onClick={() => setIsSettingsOpen(true)} className="md:hidden text-gray-400"><svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="3"></circle><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-2 2 2 2 0 0 1-2-2v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1-2-2 2 2 0 0 1 2-2h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 2-2 2 2 0 0 1 2 2v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 2 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-1.51 1z"></path></svg></button>
         </div>
         <div className="w-full md:flex-1 md:max-w-2xl md:mx-8"><SearchBar /></div>
         <button onClick={() => setIsSettingsOpen(true)} className="hidden md:block text-gray-400 hover:text-blue-600 transition-all"><svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="3"></circle><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 0 1 0 2.83 2 2 0 0 1-2.83 0l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-2 2 2 2 0 0 1-2-2v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 0 1-2.83 0 2 2 0 0 1 0-2.83l.06-.06a1.65 1.65 0 0 0 .33-1.82 1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1-2-2 2 2 0 0 1 2-2h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 0 1 0-2.83 2 2 0 0 1 2.83 0l.06.06a1.65 1.65 0 0 0 1.82.33H9a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 2-2 2 2 0 0 1 2 2v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 0 1 2.83 0 2 2 0 0 1 0 2.83l-.06.06a1.65 1.65 0 0 0-.33 1.82V9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 2 2 2 2 0 0 1-2 2h-.09a1.65 1.65 0 0 0-1.51 1z"></path></svg></button>
      </nav>
      ...
      <main className="max-w-6xl mx-auto px-6 py-12">
        ...
        <div className="mb-12 bg-white rounded-3xl p-8 border-2 border-gray-200 shadow-sm flex flex-col md:flex-row gap-10">
           <div className="w-full md:w-2/3">
             <h2 className="text-xl font-bold mb-6 flex items-center gap-2"><span className="text-green-500">📈</span> Learning Activity</h2>
             <div className="overflow-visible">
               <CalendarHeatmap
                 ...
                 tooltipDataAttrs={(value: any) => {
                   ...
                   return { "data-tooltip-id": "heatmap-tooltip", "data-tooltip-content": `${value.date}: ${value.count} words` } as any;
                 }}
        ...
        <div className="mb-12">
          <h2 className="text-xl font-bold mb-6 flex items-center gap-2"><span className="text-blue-500">🧠</span> Progress</h2>
                ...
                <div key={stat.code} className="bg-white rounded-3xl p-6 border-2 border-gray-200 shadow-sm flex flex-col items-center justify-center transition-all hover:shadow-md hover:border-blue-300">
                  ...
                  <div className="relative w-24 h-24 flex items-center justify-center">
                    <svg className="transform -rotate-90 w-24 h-24">
                      ...
                      <circle cx="48" cy="48" r={radius} stroke="#2563eb" strokeWidth="8" fill="none" strokeDasharray={circumference} strokeDashoffset={strokeDashoffset} className="transition-all duration-1000 ease-out" strokeLinecap="round" />
                    </svg>
                    <span className="absolute text-xl font-black">{stat.percentage}%</span>
                  </div>
                  <p className="text-xs font-bold text-gray-400 mt-4 uppercase tracking-widest">{stat.remembered} / {stat.total} Words</p>
                </div>
              );
            })}
          </div>
          ...
          {languages.map((lang) => (
            <Link key={lang.code} href={`/study/${lang.code}`} className="group">
              <div className="bg-white rounded-3xl p-8 border-2 border-gray-200 shadow-sm hover:border-blue-500 hover:shadow-xl transition-all h-64 flex flex-col justify-between relative overflow-hidden">
                ...
                <div className="z-10"><h3 className="text-3xl font-black tracking-tight">{lang.name}</h3><p className="text-blue-600 font-bold flex items-center gap-2 mt-2 group-hover:gap-4 transition-all uppercase text-xs tracking-widest">Start Session →</p></div>
              </div>
            </Link>
          ))}
        </div>

        <div className="mb-12"><CreateCardForm /></div>
      </main>
    </div>
  );
}